...
pub async fn warn(
    ...
    let guild_config = ctx.data().guild_configs.get(&guild_id).map(|entry| entry.clone())
        .unwrap_or_else(|| {
    ...
    let enforcement = match action.as_deref() {
        Some("mute") | Some("Mute") => Some(EnforcementAction::Mute { duration: duration * 3600 }),
        Some("ban") | Some("Ban") => Some(EnforcementAction::Ban { duration: duration * 3600 }),
        Some("kick") | Some("Kick") => Some(EnforcementAction::DelayedKick { delay: duration * 3600 }),
    ...
    // Store warning
    ctx.data().warnings.insert(warning_id.clone(), warning.clone());
    ...
    match warning.notification_method {
        NotificationMethod::DirectMessage => {
            if let Ok(channel) = user.create_dm_channel(&ctx.http()).await {
                let embed = CreateEmbed::new()
                    ...
                    .description(format!("You have been warned in {} for: {}", ctx.guild().unwrap().name, warning.reason))
                    .colour(Colour::RED)
        ...
        NotificationMethod::PublicWithMention => {
            let content = format!("{} You have been warned for: {}", user.mention(), warning.reason);
            let embed = CreateEmbed::new()
    ...
    ctx.say(format!("Warned {} for: {}", user.name, warning.reason)).await?;
    Ok(())
}
...
pub async fn cancelwarning(
    ...
    for id in pending_to_cancel {
        if let Some(mut pending) = ctx.data().pending_enforcements.get_mut(&id) {
            ...
            canceled = true;
            response.push_str(&format!("Canceled enforcement ID {} for {}\n", id, user.name));
        }
    }
