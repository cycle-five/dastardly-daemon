...
pub async fn vcthumbsdown(
    ...
    let state = ctx.data().add_to_user_warning_state(user_id, guild_id.get(), reason.clone(), mod_id);
    ...
    let enforcement_action = if state.pending_enforcement.is_some() {
        ...
        let enforcement = guild_config.default_enforcement.unwrap_or(
            EnforcementAction::VoiceMute { duration: Some(300) }
        );
    ...
    if let Some(log_channel_id) = guild_config.enforcement_log_channel_id {
        ...
        let mut embed = serenity::CreateEmbed::new()
            ...
            .description(format!("{} has received a voice channel warning", user_mention))
        ...
        if let Some(ref action) = enforcement_action {
            if state.warning_timestamps.len() == 1 {
                ...
                embed = embed.field(
                    ...
                    format!("After ~{} more warnings, the user will receive: **{}**", 
                ...
                embed = embed
                    ...
                    .field("⚠️ Threshold Reached", "Enforcement action is being applied", false);
            }
        }
    ...
    if enforce && enforcement_action.is_some() {
        ...
        let warning = Warning {
            ...
            issuer_id: mod_id,
            guild_id: guild_id.get(),
            reason: format!("Automatic enforcement after multiple voice warnings: {}", reason),
            timestamp: now.clone(),
        ...
        if let Some(action) = enforcement_action {
            ...
            ctx.data().pending_enforcements.insert(enforcement_id, pending);
    ...
    let response = if enforce {
        format!("Thumbs down recorded for {} with reason: {}. Enforcement action applied due to multiple warnings.", user.name, reason)
    } else if state.warning_timestamps.len() == 1 {
        format!("First thumbs down recorded for {} with reason: {}. Further warnings may trigger enforcement.", user.name, reason)
    } else {
        format!("Thumbs down recorded for {} with reason: {}. Current warning count: {}", 
            user.name, reason, state.warning_timestamps.len())
    };
...
pub async fn setenforcementlog(
    ...
    // Save data
    if let Err(e) = ctx.data().save().await {
        error!("Failed to save data after setting enforcement log channel: {}", e);
        ctx.say("Failed to save configuration. Check logs for details.").await?;
        return Ok(());
    }
    ...
    match channel_id.send_message(&ctx.http(), message).await {
        Ok(_) => {
            ctx.say(format!("Successfully set {} as the enforcement log channel!", channel.mention()))
                .await?;
        },
        Err(e) => {
