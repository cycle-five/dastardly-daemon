...
impl EnforcementService {
    ...
    pub fn start_task_with_receiver(
        ...
        tokio::spawn(async move {
            self.enforcement_task(http, rx, check_interval_seconds)
                .await;
        });
    }
            ...
            match state {
                EnforcementState::Pending => {
                    if let Ok(record) = self.store.execute_enforcement(&enforcement_id) {
                        ...
                        info!("Executing enforcement record id {record_id}");
                        // Execute the action
                        ...
                        let result = self
                            .handlers
                            .execute(http, guild_id, user_id, &action)
                            .await;
                        ...
                        { self.store.get(&enforcement_id).and_then(|r| r.reverse_at) }
                    {
                        if reverse_at <= chrono::Utc::now() {
                            if let Ok(record) = self.store.reverse_enforcement(&enforcement_id) {
                                let record_id = record.id.clone();
                                info!("Reversing enforcement record id {record_id}");
                                // Reverse the action
                                let guild_id = GuildId::new(guild_id);
                                let user_id = UserId::new(user_id);
                                ...
                                let result = self
                                    .handlers
                                    .reverse(http, guild_id, user_id, &action)
                                    .await;
    ...
    pub async fn check_user_enforcements(
        ...
        for record in &pending {
            if record.is_due_for_execution() {
                if let Err(e) = self.process_enforcement(http, &record.id).await {
                    error!(
                        "Failed to process pending enforcement {} for user {}: {}",
                        record.id, user_id, e
                    );
                }
            }
        ...
        for record in &active {
            if record.is_due_for_reversal() {
                if let Err(e) = self.process_enforcement(http, &record.id).await {
                    error!(
                        "Failed to process active enforcement {} for user {}: {}",
                        record.id, user_id, e
                    );
                }
            }
        ...
        if let Some(tx) = &*self.tx {
                ...
                .await
            {
                ...
                return Err(EnforcementError::Other(format!(
                    "Failed to send user check request: {e}"
                )));
            }
        } else {
    ...
    pub async fn notify_about_enforcement(&self, enforcement_id: &str) -> EnforcementResult<()> {
        if let Some(tx) = &*self.tx {
                ...
                .await
            {
                ...
                return Err(EnforcementError::Other(format!(
                    "Failed to send enforcement check request: {e}"
                )));
            }
        } else {
            return Err(EnforcementError::Other(
    ...
    pub async fn notify_check_all(&self) -> EnforcementResult<()> {
        if let Some(tx) = &*self.tx {
            if let Err(e) = tx.send(EnforcementCheckRequest::CheckAll).await {
                ...
                return Err(EnforcementError::Other(format!(
                    "Failed to send check all request: {e}"
                )));
            }
        } else {
    ...
    pub fn import_and_start(&mut self, _data: &Data, http: Arc<Http>, check_interval_seconds: u64) {
        ...
        if let Some(rx) = Self::take_enforcement_receiver() {
            ...
            self.clone()
                .start_task_with_receiver(http, rx, check_interval_seconds);
        } else {
